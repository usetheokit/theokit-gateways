...
- 1e215ab: The peer floor on `@theokit/gateway` is `>=0.9.0`, which is what these packages have needed since
  they gained `deliver`.
...
- 72f5807: An email with no display name no longer carries an empty one.
...
- aa2eced: The nodemailer peer range no longer admits only vulnerable versions.
