...
- **Fixed:** the same READMEs stated that a TheoKit app's `onMessage` runs *after* the 200 is answered. Measured against `theokit@0.48.14`, it is awaited before the response is built with nothing catching around it, so a throw there escapes the route and answers an error where the platform expected an acknowledgement. `parseInbound` returning `null` is unchanged and still right; the reason given for it was wrong, and had reached two READMEs, a source comment and an ADR (#B-011)
- **Fixed:** the READMEs implied every adapter exports `parseInbound`. Only `@theokit/gateway-telegram` and `@theokit/gateway-sms` do; the rest export their translation under their own names and return `undefined` rather than `null` (#B-011)
...
- **gateway-telegram, gateway-sms:** `parseInbound(payload)` — an app can translate a raw webhook body into a canonical event without re-declaring the platform's wire format. Measured before: an app wiring Telegram wrote 35 lines, 27 of them platform knowledge. Every field the translator copies is narrowed at the boundary, so a payload with `text: 5` yields an event whose `text` is `""` rather than the number 5, and one with `date: NaN` is rejected outright rather than producing `receivedAt: NaN`. `gateway-line` and `gateway-whatsapp` already shipped this shape and are unchanged (#B-009)
