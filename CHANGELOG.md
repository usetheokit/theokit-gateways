...
### Added
- **all adapters:** every credential field now documents the platform's own term for it and where it is issued, on the published option types — so an editor answers "which field?" on hover instead of the developer opening the type. Where our name diverges from the platform's SDK (`botToken`, mattermost's `accessToken`, `password`) the docblock says so. A cross-adapter gate fails when any of the ten omits it (#B-010)
