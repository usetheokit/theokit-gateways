...
### Added
- **gateway-telegram, gateway-sms:** `parseInbound(payload)` — an app can translate a raw webhook body into a canonical event without re-declaring the platform's wire format. Measured before: an app wiring Telegram wrote 35 lines, 27 of them platform knowledge. Every field the translator copies is narrowed at the boundary, so a payload with `text: 5` or `date: NaN` is rejected rather than producing an event whose declared types are lies. `gateway-line` and `gateway-whatsapp` already shipped this shape and are unchanged (#B-009)
