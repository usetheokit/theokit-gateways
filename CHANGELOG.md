...
### Added
- **ci:** `Promotion gate` refuses a pull request into `develop` whose head is not `workspace`. `git-safety.md` has always said so and `validate-command.sh` has always blocked it — for a `git merge` typed locally, which is not how any of this repository's 43 promotions landed. A sibling repository carrying the identical rule and hook accumulated thirty merged `main → develop` pull requests through the same gap
...
- **every adapter now carries a measured mutation ratchet.** The ten first measurements averaged
  ...
  with processing time and could not tell a group message's sender from nothing, Matrix registered
  its timeline listener under an event name no test read (reaching 100%), SMS rejected a
  too-small limit with an internal invariant instead of the operator's own number, and Telegram's
  markdown-pair balancing — the whole EC-J feature — was undetectable because both of its tests
  built input that was already balanced everywhere (#B-024)
...
- **gateway:** the EC-E drain test asserted a fact it got for free. It awaited the handler promise
  ...
  from inside the handler, which is the only moment "before disconnect" can be observed.
...
### Added
- **gateway-whatsapp:** the unofficial (Baileys) backend now answers a note the account owner writes to themselves, instead of discarding every message the paired account sent (#84)
- **gateway-whatsapp:** `WhatsAppBaileysBackend.pairing` reports what a pairing is doing — `idle` / `awaiting_scan` / `connected` / `closed`, plus the current QR and when it arrived. `onQr` is push, firing at a moment the caller does not choose; a screen is pull, loading whenever someone opens it. Without a readable state every consumer kept its own copy of the latest code and its own notion of whether pairing had succeeded, which is this backend's bookkeeping copied into each of them. The QR is dropped on `connected` and on `closed`, because WhatsApp reissues roughly every 20 seconds and a screen holding the previous square offers something that cannot be scanned. Rendering stays in the app: `qrcode` pulls a CLI argument parser, and a consumer using only the Cloud API would carry it to pair nothing

- **repo:** `theokit` is a root devDependency so `quality:doc-api` can verify the framework symbols our READMEs tell a reader to import. It reported them honestly before — `6 import(s) NOT checked — package not installed here` — which is better than a silent pass and still leaves the claim resting on one manual check that does not survive the next release. The gate went from 32 verified names to 40, and the "NOT checked" block is now empty. Proved failing on the exact defect that motivated it: a sibling repository shipped `defineAgentTool` in its docs, typable and absent at runtime (usetheokit/theokit#542, reopened 2026-08-29), and substituting that name into our example produces `theokit/server/webhook has no 'defineAgentTool'` and exit 1. Costs 39 MB in a dev install and nothing to a consumer

- **docs:** both READMEs now show how to receive WhatsApp through TheoKit's channel seam, which `theokit@0.60.0` made possible for the first time (usetheokit/theokit#556, filed from here — the `validators` map could not carry a `whatsapp` entry, so the path answered 404 by construction, and Meta's subscribe handshake had no seam at all). The example names the three things that are easy to miss: mount **both** verbs because Meta's `hub.mode=subscribe` handshake is a GET, declare `.csrf(false)` on each or the route answers 403 before the signature is read, and pass the Meta **app secret** rather than the access token. It also says which of our symbols does the translation — `parseWebhookPayload` takes the `unknown` payload `onMessage` hands over — and why this package still exports its own `verifyWebhookSignature` / `verifyWebhookSubscription`: for an app that is not on the seam, which is how `gateway-sms` is wired. What was verified here is stated separately from what was taken from the session that shipped 0.60.0
