...
it: a throw escapes it entirely and the 200 is never built. `parseInbound` returning `null` is
unchanged and still the right contract — only the stated reason was wrong, and it was reaching users
through the `.d.ts` on hover.
