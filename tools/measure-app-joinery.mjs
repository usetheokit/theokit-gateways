...
const FILES = [
  ...
  ['server/routes/agents/[agent]/channels/[platform]/webhook.ts', 'gap', 'per-platform parse + send dispatch'],
...
process.stdout.write(JSON.stringify({ metric: 'lines neither blank nor comment-only', root, rows, byOwner, total }, null, 2) + '\n')
